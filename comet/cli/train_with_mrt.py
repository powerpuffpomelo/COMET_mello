...
def train_comet(dataset=None, lr=3e-5, pretrained_model=None) -> None:
    ...
    cfg = parser.parse_args(['--cfg', 'COMET_mello/configs/configs_for_step_finetune/wmt20-comet-da.yaml'])
    cfg.regression_metric.init_args.learning_rate = lr
    ...
    if cfg.regression_metric is not None:
        ...
        if pretrained_model is not None:
            cfg.regression_metric.init_args.pretrained_model=pretrained_model
        params = namespace_to_dict(cfg.regression_metric.init_args)
        params['dataset_already'] = True
        params['train_dataset'] = dataset
        params['validation_dataset'] = dataset
        model = RegressionMetric(**params)  # 加载模型
    ...
    ckpt_path_prefix = cfg.model_checkpoint.init_args.dirpath + '/' + cfg.model_checkpoint.init_args.filename
    os.rename(ckpt_path_prefix + '-v1.ckpt', ckpt_path_prefix + '.ckpt')
