...
def load_from_checkpoint(checkpoint_path: str, pretrained_model=None) -> CometModel:
    ...
    if os.path.exists(hparams_file):
        with open(hparams_file) as yaml_file:
            ...
            if pretrained_model is not None:
                hparams['pretrained_model'] = pretrained_model
...
def load_from_checkpoint_wo_hparams(checkpoint_path: str) -> CometModel:
    """Loads models from a checkpoint path.
    :param checkpoint_path: Path to a model checkpoint.
    ...
    model_class = str2model['regression_metric']
